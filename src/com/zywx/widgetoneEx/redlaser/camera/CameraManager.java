...
public final class CameraManager {
     ...
     */
    /*
     * public Rect getFramingRect() { Point screenResolution = configManager.getScreenResolution(); if (framingRect ==
     * null) { if (camera == null) { return null; } int width = screenResolution.x * 2 / 3; if (width < MIN_FRAME_WIDTH)
     * { width = MIN_FRAME_WIDTH; } else if (width > MAX_FRAME_WIDTH) { width = MAX_FRAME_WIDTH; } int height =
     * screenResolution.y * 2 / 3; if (height < MIN_FRAME_HEIGHT) { height = MIN_FRAME_HEIGHT; } else if (height >
     * MAX_FRAME_HEIGHT) { height = MAX_FRAME_HEIGHT; } int leftOffset = (screenResolution.x - width) / 2; int topOffset
     * = (screenResolution.y - height) / 4; framingRect = new Rect(leftOffset, topOffset, leftOffset + width, topOffset
     * + height); } return framingRect; }
     */
    ...
    public PlanarYUVLuminanceSource buildLuminanceSource(byte[] data, int width, int height) {
        ...
        // return new PlanarYUVLuminanceSource(Bitmap2Bytes(bmp),
        // bmp.getWidth(),
        // bmp.getHeight());
