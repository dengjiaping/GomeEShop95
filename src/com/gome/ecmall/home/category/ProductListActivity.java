...
/**
 * 分类商品列表 筛选后有新ProductFilterListActivity类似结构 如修改，两处都对等修改
 */
public class ProductListActivity extends AbsSubActivity implements
	...
	private final int[] LABEL_IDS = new int[] { R.string.product_hot,
			R.string.product_sale, R.string.product_price,
			R.string.product_time };
	private final int[] PRODUCT_ORDERS = new int[] { Product.SORTBY_HOT,
			Product.SORTBY_SALE_DESC, Product.SORTBY_PRICE_ASC,
			Product.SORTBY_TIME_DESC };
	private final int[] TAB_BTNS_ID = new int[] {
			R.id.product_list_tab_item_hot, R.id.product_list_tab_item_sale,
	...
	protected void onResume() {
		...
		if (getIntent() != null) {
			...
			AppMeasurementUtils appMeasurementUtils = new AppMeasurementUtils(
					ProductListActivity.this);
			appMeasurementUtils.getUrl(getString(R.string.appMeas_productlist)
					+ ":" + title, getString(R.string.appMeas_productlist),
					getString(R.string.appMeas_productlist) + ":" + title,
					getString(R.string.appMeas_productlist_page), fromPage, "",
					"", "", "", "", "", "",
					getString(R.string.appMeas_categoryPage), "", "", "", null);
			// 修改分类返回状态
			if (!TextUtils.isEmpty(fromPage)) {
				CategoryActivity.isPosted = true;
			}
		}
	}
	...
	private void setupView() {
		...
		if ("PromtionActivitiesActivity".equals(getIntent().getAction())) {
			tvTitle.setText(getIntent().getStringExtra(
					JsonInterface.JK_ACTIVITY_NAME));
		...
		layoutSwitch = (LinearLayout) inflater.inflate(
				R.layout.common_product_display_switch_layout, null);
		btnGird = (Button) layoutSwitch
				.findViewById(R.id.category_product_display_type_grid);
		btnList = (Button) layoutSwitch
				.findViewById(R.id.category_product_display_type_list);
		listView = (ListView) findViewById(R.id.category_lv_product_list);
		// controller = AnimationUtils.getLayoutAnimationController() ;
		loadingView = (LinearLayout) inflater.inflate(
		...
		downDrawable = getResources().getDrawable(
				R.drawable.product_rank_down_bg_selector);
		upDrawable = getResources().getDrawable(
				R.drawable.product_rank_up_bg_selector);
		initRankTabs();
	}
	...
	private void reloadData(final String goodsTypeId, final int sortBy,
			final ArrayList<FilterCondition> conditions) {
		if (!NetUtility.isNetworkAvailable(ProductListActivity.this)) {
			CommonUtility
					.showMiddleToast(
							ProductListActivity.this,
							"",
		...
		new AsyncTask<Object, Void, ArrayList<Product>>() {
			...
			protected void onPreExecute() {
				loadingDialog = CommonUtility.showLoadingDialog(
						ProductListActivity.this, getString(R.string.loading),
			...
			protected ArrayList<Product> doInBackground(Object... params) {
				...
				if ("PromtionActivitiesActivity".equals(intent.getAction())) {// 判断是否是活动专题
					String activityId = intent
							.getStringExtra(JsonInterface.JK_ACTIVITY_ID);
					String activityType = intent
							.getStringExtra(JsonInterface.JK_ACTIVITY_TYPE);
					String activityHtmlurl = intent
							.getStringExtra(JsonInterface.JK_ACTIVITY_HTML_URL);
					String request = LimitBuyResult
							.createRequestLimitBuyPrmListJson(activityId,
									activityType, activityHtmlurl);
					response = NetUtility.sendHttpRequestByPost(
							Constants.URL_ACTIVITIES_GENERAL_ACTIVITY_GOODS,
							request);
					...
					String request = Product.createRequestProductListJson(
							goodsTypeId, currentPage, Constants.PAGE_SIZE,
							sortBy, conditions);
					if (conditions != null && conditions.size() > 0) {// 有筛选条件
						response = NetUtility.sendHttpRequestByPost(
								Constants.URL_PRODUCT_LIST, request);
					} else {// 没有筛选条件
						response = NetUtility.sendHttpRequestByPost(
								Constants.URL_PRODUCT_TYPE_SEARCH, request);
					}
					if (NetUtility.NO_CONN.equals(response)) {
						return null;
					}
			...
			protected void onPostExecute(ArrayList<Product> result) {
				...
				if (result == null) {
					...
					CommonUtility.showMiddleToast(ProductListActivity.this, "",
							getString(R.string.data_load_fail_exception));
				} else {
					products = result;
				}
				if (!"PromtionActivitiesActivity".equals(getIntent()
						.getAction())) {
					if (products.size() < Constants.PAGE_SIZE) {// 没有商品可取
						hasMoreData = false;
						((TextView) loadingView
								.findViewById(R.id.common_loading_title))
				...
				if (productListAdapter == null) {
					productListAdapter = new ProductListAdapter(
							ProductListActivity.this, products, calssType);
					listView.setAdapter(productListAdapter);
					// listView.setLayoutAnimation(controller) ;
					tvTitle.setOnClickListener(ProductListActivity.this);
					btnFilter.setOnClickListener(ProductListActivity.this);
					btnGird.setOnClickListener(ProductListActivity.this);
					btnList.setOnClickListener(ProductListActivity.this);
					tvEmpty.setText(R.string.empty);
					listView.setEmptyView(tvEmpty);
					// list item
					// 的点击事件此处没有设置，取而代之的是给adapter自定义的监听OnProductClickListener
					// listView.setOnItemClickListener(ProductListActivity.this);
					listView.setOnScrollListener(ProductListActivity.this);
					productListAdapter
							.setClickListener(new OnProductClickListener() {
								...
								public void onProductClick(Product product) {
									...
									intent.putExtra(
											ProductShowActivity.INTENT_KEY_GOODS_NO,
											product.getGoodsNo());
									intent.putExtra(
											"fromPage",
											getString(R.string.appMeas_categoryPage));
									recordProductBrowseHistory(product);
									startActivity(intent);
								}
							});
					for (int i = 0; i < tabBtns.length; i++) {
						tabBtns[i].setOnClickListener(ProductListActivity.this);
					}
	 ...
	 *            排序方式
	 */
	private void loadMoreData(final String goodsTypeId, final int page,
			final int sortBy, final ArrayList<FilterCondition> conditions) {
		if (!NetUtility.isNetworkAvailable(ProductListActivity.this)) {
			CommonUtility
					.showMiddleToast(
							ProductListActivity.this,
		...
		asyncTask = new AsyncTask<Object, Void, ArrayList<Product>>() {
			...
			protected ArrayList<Product> doInBackground(Object... params) {
				String response = NetUtility.NO_CONN;
				String request = Product.createRequestProductListJson(
						goodsTypeId, page, Constants.PAGE_SIZE, sortBy,
						conditions);
	...
	private void initFilterDialog() {
		...
		new AsyncTask<Object, Void, ArrayList<FilterCondition>>() {
			LoadingDialog loadingDialog;

			@Override
			protected void onPreExecute() {
				loadingDialog = CommonUtility.showLoadingDialog(
						ProductListActivity.this, "正在获取筛选条件...", true,
			...
			@Override
			protected ArrayList<FilterCondition> doInBackground(
					Object... params) {
				String request = Product
						.createRequestProductFilterConditionJson(goodsTypeId);
				String response = NetUtility.sendHttpRequestByPost(
						Constants.URL_PRODUCT_TYPE_SEARCH_CON, request);
				if (NetUtility.NO_CONN.equals(response)) {
					return null;
				}
				return Product.parseFilterConditionList(response);
			}

			@Override
			protected void onPostExecute(final ArrayList<FilterCondition> result) {
				if (isCancelled()) {
					return;
				}
				loadingDialog.dismiss();
				...
				if (filterConditions.size() == 0) {
					CommonUtility.showToast(ProductListActivity.this, "没有筛选条件");
					return;
				}
				filterDialog = new FilterDialog(ProductListActivity.this);
				filterDialog.show();
				filterDialog.setTitle(getString(R.string.filter));
				filterDialog.setButtonListener(getString(R.string.confirm),
						new View.OnClickListener() {
							...
							public void onClick(View v) {
								...
								// 判断是否有筛选条件
								boolean flag = false;
								for (int i = 0, size = filterConditions.size(); i < size; i++) {
									if (filterConditions.get(i)
								...
								if (flag) {
									...
									intent.setClass(ProductListActivity.this,
											ProductFilterListActivity.class);
									intent.putExtra(
											ProductListActivity.INTENT_KEY_TITLE,
											title);
									intent.putExtra(
											ProductListActivity.INTENT_KEY_GOODS_TYPE_ID,
											goodsTypeId);
									intent.putExtra(
											ProductListActivity.INTENT_KEY_CURRENT_CHECK_INDEX,
											checkedIndex);
									intent.putExtra(
											ProductListActivity.INTENT_KEY_FLITER_CONDITION_LIST,
											filterConditions);
									intent.putExtra(
											ProductListActivity.INTENT_KEY_IS_GRID_MODE,
											productListAdapter
													.getItemViewType(0) == MODE_GRID ? true
													: false);
									startActivity(intent);
								}
				...
				filterDialog.setData(result, new OnChildClickListener() {

					@Override
					public boolean onChildClick(ExpandableListView parent,
							View v, int groupPosition, int childPosition,
							long id) {
						FilterConditionsAdapter adapter = (FilterConditionsAdapter) parent
								.getExpandableListAdapter();
						FilterCondition filterCondition = adapter
	...
	public void onScroll(AbsListView view, int firstVisibleItem,
		...
		if (!"PromtionActivitiesActivity".equals(intent.getAction())) {
			if (firstVisibleItem + visibleItemCount == totalItemCount) {// 滑动到底部
				if (hasMoreData && productListAdapter != null
						&& productListAdapter.getCount() > 0) {
					loadMoreData(goodsTypeId, currentPage, currentSortBy,
							filterConditions);
				}
			}
		}
	...
	@Override
	public void onTabSelectChanged(SegmentTabs tabs, View item, int lastIndex,
			int currentIndex) {
