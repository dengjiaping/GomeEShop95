...
/**
 * 排行榜筛选列表 来源为RankingListActivity类似结构 如修改，两处都对等修改
 */
public class RankingFilterListActivity extends AbsSubActivity implements OnClickListener, OnItemClickListener,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        categoryId = getIntent().getStringExtra(INTENT_KEY_CATEGORY_ID);
        checkedIndex = getIntent().getIntExtra(INTENT_KEY_CURRENT_CHECK_INDEX, -1);
        softConList = (ArrayList<FilterType>) getIntent().getSerializableExtra(INTENT_KEY_FLITER_CONDITION_LIST);
        setupView();
        setupData();
    }
    ...
    private void setupData() {
        currentSortType = RANKING_ORDERS[checkedIndex];
    ...
    private void reloadData(final String categoryId, final int currentSortType, final ArrayList<FilterType> conditions) {
        if (!NetUtility.isNetworkAvailable(RankingFilterListActivity.this)) {
            CommonUtility.showMiddleToast(RankingFilterListActivity.this, "",
        ...
        new AsyncTask<Object, Void, ArrayList<Ranking>>() {
            ...
            protected void onPreExecute() {
                loadingDialog = CommonUtility.showLoadingDialog(RankingFilterListActivity.this,
            ...
            protected ArrayList<Ranking> doInBackground(Object... params) {
                String request = Ranking.createRequestRankingListJson(categoryId, currentPage, Constants.PAGE_SIZE,
            ...
            protected void onPostExecute(ArrayList<Ranking> result) {
                ...
                if (result == null) {
                    ...
                    if (TextUtils.isEmpty(Ranking.getErrorMessage())) {
                        ...
                        CommonUtility.showMiddleToast(RankingFilterListActivity.this, "", Ranking.getErrorMessage());
                    }
                } else {
                    rankings = result;
                }
                ...
                if (rankingListAdapter == null) {
                    rankingListAdapter = new RankingListAdapter(RankingFilterListActivity.this, rankings,
                            currentSortType);
                    listView.setAdapter(rankingListAdapter);
                    tvTitle.setOnClickListener(RankingFilterListActivity.this);
                    common_title_btn_right.setOnClickListener(RankingFilterListActivity.this);
                    tvEmpty.setText(R.string.empty);
                    listView.setEmptyView(tvEmpty);
                    listView.setOnItemClickListener(RankingFilterListActivity.this);
                    listView.setOnScrollListener(RankingFilterListActivity.this);
                    for (int i = 0; i < tabBtns.length; i++) {
                        tabBtns[i].setOnClickListener(RankingFilterListActivity.this);
                    }
                } else {
    ...
    private void initSoftDialog() {
        if (softConList != null) {
            ...
            new AsyncTask<Object, Void, FilterSoft>() {
                ...
                @Override
                protected void onPreExecute() {
                    loadingDialog = CommonUtility.showLoadingDialog(RankingFilterListActivity.this, "正在获取筛选条件...",
    ...
    private void initSoftDialogView() {
        if (softConList == null || softConList.size() == 0) {
            CommonUtility.showToast(RankingFilterListActivity.this, "没有筛选条件");
            return;
        }
        softDialog = new SoftDialog(RankingFilterListActivity.this);
        softDialog.show();
