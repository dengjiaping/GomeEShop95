...
/**
 * 套购主页筛选对话框 另有套购筛选页筛选对话框，两处结构类似，如有修改，两处对等修改
 * 
 */
public class SuiteFilterDialog extends Dialog {
    ...
    public SuiteFilterDialog(Context context) {
        super(context, R.style.Style_filter_dialog);
    }

    public SuiteFilterDialog(Context context, int theme) {
        super(context, theme);
    }
    ...
    public void setData(Context ctx, ArrayList<SuiteBuyFilter> filters, SuiteFilterDialog dialog) {
        adapter = new MyFilterAdapter(ctx, filters);
        mListView.setAdapter(adapter);
        // mListView.setOnItemClickListener(new ItemClick(filters));
        mConfirmBtn.setOnClickListener(new ClickListener(dialog, filters));
        mCancelBtn.setOnClickListener(new ClickListener(dialog, filters));
    }

    public static SuiteFilterDialog show(Context context, ArrayList<SuiteBuyFilter> filters) {
        SuiteFilterDialog dialog = new SuiteFilterDialog(context);
        dialog.show();
    ...
    class ClickListener implements android.view.View.OnClickListener {
        SuiteFilterDialog mDialog;
        ArrayList<SuiteBuyFilter> mFilters;

        public ClickListener(SuiteFilterDialog dialog, ArrayList<SuiteBuyFilter> filters) {
            mDialog = dialog;
            mFilters = filters;
        }
        ...
        public void onClick(View v) {
            ...
            case R.id.filter_dialog_btn:
                ...
                if (!TextUtils.isEmpty(getSelectIndex())) {
                    Intent intent = new Intent();
                    intent.setClass(mActivity, SuiteBuyFilterActivity.class);
                    intent.putExtra(mActivity.INTENT_KEY_SELECT_INDEX, getSelectIndex());
                    intent.putExtra(mActivity.INTENT_KEY_FILTER_LIST, mFilters);
