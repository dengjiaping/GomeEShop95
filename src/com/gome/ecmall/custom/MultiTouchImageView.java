...
public class MultiTouchImageView extends ImageView {
    ...
    private GestureDetector gestureDetector = new GestureDetector(new SimpleOnGestureListener() {
        ...
        public boolean onDoubleTap(MotionEvent event) {
            ...
            if (currentScale > getScale(baseMatrix)) {// 放大状态,双击变成初始状态
                AnimZoomTo(getScale(baseMatrix), event.getX(), event.getY(), getTransX(baseMatrix),
    ...
    public void setImageBitmap(Bitmap bm) {
        // if (bm != null) {
        // setFitScreenMatrix(bm.getWidth(), bm.getHeight(), baseMatrix);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        case MotionEvent.ACTION_POINTER_DOWN:
            ...
            mode = ZOOM;
            if (bottomView != null) {
                bottomView.setVisibility(View.GONE);// 隐藏底部视图
        ...
        case MotionEvent.ACTION_POINTER_UP:// 第2个手指离开
            ...
            if (currentScale <= baseScale) {// 缩小状态，需要弹回原始大小
                ...
                if (currentScale > MAX_SCALE_RATE) {// 大于最大缩放率，弹回最大缩放率
                    ...
                    final Matrix tmpMatrix = new Matrix(currentMatrix);
                    tmpMatrix.postScale(deltaScale, deltaScale, midPoint.x, midPoint.y);
                    final float targetTransX = getTransX(tmpMatrix);
                    final float targetTransY = getTransY(tmpMatrix);
                    AnimZoomTo(MAX_SCALE_RATE, midPoint.x, midPoint.y, targetTransX, targetTransY, 300);
                }
            }
            mode = NONE;
            return false;
    ...
    private void zoomTo(float targetScale, float centerX, float centerY, float targetTransX, float targetTransY) {
        ...
        currentMatrix.postTranslate(deltaTransX - (getTransX(currentMatrix) - currentTransX), deltaTransY
                - (getTransY(currentMatrix) - currentTransY));
        setImageMatrix(currentMatrix);
    }

    private void animTranslateTo(final float destX, final float destY, final float duration) {
        ...
        mHandler.post(new Runnable() {
            public void run() {
                ...
                final float deltaTransX = targetTransX - getTransX(currentMatrix);// 目标X轴偏移量与当前X轴偏移量的差量
                final float deltaTransY = targetTransY - getTransY(currentMatrix);// 目标Y轴偏移量与当前Y轴偏移量的差量
    ...
    private void AnimZoomTo(final float destScale, final float centerX, final float centerY, final float destX,
            final float destY, final float duration) {
    ...
    public String getMatrixInfo(Matrix matrix) {
        return "--> Scale:" + getValue(matrix, Matrix.MSCALE_X) + " TransX:" + getValue(matrix, Matrix.MTRANS_X)
                + "  TransY:" + getValue(matrix, Matrix.MTRANS_Y);
    }
    ...
    public void ZoomToBase() {
        if (getCurrentScale() > getScale(baseMatrix)) {// 放大状态,双击变成初始状态
            AnimZoomTo(getScale(baseMatrix), getWidth() / 2, getHeight() / 2, getTransX(baseMatrix),
                    getTransY(baseMatrix), 300);
        }
    }
