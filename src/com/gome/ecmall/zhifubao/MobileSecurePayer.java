...
public class MobileSecurePayer {
    ...
    public boolean pay(final String strOrderInfo, final Handler callback, final int myWhat, final Activity activity,
        ...
        Intent intent = null;
        if (flag) {
            intent = new Intent("com.alipay.android.app.IAlixPay");// 支付宝插件
        } else {
            intent = new Intent("com.eg.android.AlipayGphone.IAlixPay");// 支付宝钱包快捷支付
        }
        // bind the service.
        if (mAlixPay == null) {
            // mActivity.bindService(new Intent(IAlixPay.class.getName()), mAlixPayConnection,
            // Context.BIND_AUTO_CREATE);
            mActivity.bindService(intent, mAlixPayConnection, Context.BIND_AUTO_CREATE);
        }
        // else ok.
